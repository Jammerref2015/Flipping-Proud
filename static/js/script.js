$(document).ready(function() {
    $.when($.ajax("static/data/data.json")).then(function(data) {
        for (let i = 0; i < data.length; i++) {
            ...
            flipCardFront += `<div class='d-none d-md-block name-wrapper'><h5 class='pt-2 pl-3 name-wrapper-heading'>${data[i].name}</h5></div>`;
            ...
            flipCardBack += "<img src='static/images/pride-doughnut-icon.png' width='80' class='icon pride-doughnut-icon pride-doughnut-icon-back mb-2 d-md-none' alt='Pride doughnut icon'>";
